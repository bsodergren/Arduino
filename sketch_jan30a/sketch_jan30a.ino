...
unsigned long t=2000,f,k=512;// default 1000 μs (1000 Hz), meander, pulse 
...
{ 
  ...
  pinMode(8,INPUT_PULLUP);// button at input 13
}
void loop()
{
  ...
  kn2=digitalRead(8);// button input 13 (+ circle fill factor)
  if(kn==LOW){ // decreasing the period 
    drive++;
    if(drive<30){ 
      t=t-1;  
    }
  ...
  if(kn2==LOW){// button for adjusting the fill factor (in a circle from 
